-- Capture the payer's manually entered town as a transaction-time snapshot.
ALTER TABLE public.transactions
  ADD COLUMN IF NOT EXISTS payer_town varchar(120);

COMMENT ON COLUMN public.transactions.payer_town IS
  'Customer town supplied during checkout. Suggestions are provided client-side, but manual values are allowed.';

-- Replace the basket transaction RPC so town is validated and stored atomically.
DROP FUNCTION IF EXISTS public.create_qr_basket_transaction(text, text, text, text, uuid);
DROP FUNCTION IF EXISTS public.create_qr_basket_transaction(text, text, text, text, text, uuid);
...
BEGIN
  ...
  END IF;
  IF length(trim(coalesce(p_town, ''))) < 1 OR length(trim(p_town)) > 120 THEN
...
REVOKE ALL ON FUNCTION public.create_qr_basket_transaction(text, text, text, text, text, uuid)
  FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.create_qr_basket_transaction(text, text, text, text, text, uuid)
  TO service_role;
