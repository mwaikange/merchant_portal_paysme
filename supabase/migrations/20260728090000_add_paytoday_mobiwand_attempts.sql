CREATE TABLE IF NOT EXISTS public.mobiwand_payment_attempts (
  ...
  CONSTRAINT mobiwand_payment_attempts_provider_check
    CHECK (provider_key IN ('mtc_maris', 'paypulse', 'paytoday')),
  CONSTRAINT mobiwand_payment_attempts_status_check
    CHECK (status IN ('initiated', 'processing', 'paid', 'failed', 'cancelled', 'timed_out')),
  CONSTRAINT mobiwand_payment_attempts_amount_check CHECK (amount > 0),
  CONSTRAINT mobiwand_payment_attempts_currency_check CHECK (currency = 'NAD')
);
...
DROP TRIGGER IF EXISTS trg_mobiwand_payment_attempts_updated_at
...
DROP POLICY IF EXISTS "Service role full access to MobiWand attempts"
  ON public.mobiwand_payment_attempts;
...
COMMENT ON TABLE public.mobiwand_payment_attempts IS
  'Server-side MobiWand handoff and callback state for MTC Maris, PayPulse and PayToday payments.';
