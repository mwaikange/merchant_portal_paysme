-- Keep merchant registration working while older cached/live clients still
-- submit the original camelCase signup metadata without an explicit role.
...
BEGIN
  IF v_role IS DISTINCT FROM 'merchant'
     AND NOT (v_role IS NULL AND v_has_merchant_form) THEN
    RETURN NEW;
  END IF;
